...
function CalloutCard({
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    if (
      INTERACTIVE.includes(t.tagName) ||
  ...
  useEffect(() => {
    ...
    const place = () => {
      ...
      setTip({
        ...
        left: flip ? r.left - TIP_W - 8 : r.right + 8,
        flip,
      });
    };
    place();
      ...
      {active &&
        ...
        createPortal(
          ...
          >
            ...
            <div
              className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rotate-45 bg-[#0c1126]"
              style={
                tip.flip
                  ? {
                      right: -5,
                      borderTop: "1px solid rgba(20,184,166,0.4)",
                      borderRight: "1px solid rgba(20,184,166,0.4)",
                    }
                  : {
                      left: -5,
            ...
            <div className="bg-[#0c1126] border border-teal-500/40 rounded-xl p-3.5 shadow-2xl shadow-black/60">
              <p className="text-xs font-semibold text-teal-300 mb-1.5">
                {title}
              </p>
...
const CALLOUTS: Record<Role, { title: string; desc: string }[]> = {
  student: [
    {
      title: "Success Score",
      desc: "AI-generated daily score from grades, attendance, and engagement. Falls below 65 → advisor is automatically notified.",
    },
    {
      title: "Course Risk Flags",
      desc: "Color-coded per course. Green is healthy, yellow needs attention, red is critical. Ensures issues are noticed before final grades are impacted.",
    },
    {
      title: "Advisor Connect",
      desc: "One-click scheduling links to your advisor's live calendar. No email chains, availability is shown in real time.",
    },
    {
      title: "AI Action Plan",
      desc: "Personalized next steps ranked by urgency. Regenerated each evening from fresh Canvas and SIS data.",
    },
  ],
  advisor: [
    {
      title: "Live Risk Feed",
      desc: "Student list refreshes every 15 min from Canvas, SIS, and attendance. Sort by risk score, major, or last contact date.",
    },
    {
      title: "Risk Score Breakdown",
      desc: "Click any student to see which factors are driving their score: missed assignments, absences, grade drops, or disengagement.",
    },
    {
      title: "Alert Triage Queue",
      desc: "Red = act today, Yellow = monitor this week. Each alert includes an AI-suggested outreach message to send.",
    },
    {
      title: "Intervention Log",
      desc: "Every meeting, referral, and email is timestamped here. Ensures continuity of care if a student changes advisors.",
    },
  ],
  professor: [
    {
      title: "Attendance Trend",
      desc: "Week-over-week rate calculated automatically from LMS logins and in-class check-ins. A >10% drop flags the advising office.",
    },
    {
      title: "Assignment Heatmap",
      desc: "Shows which specific assignments are causing grade drops; helps separate content gaps from student engagement failures.",
    },
    {
      title: "Flag to Advising",
      desc: "One click routes a student to their advisor's alert queue with your context notes pre-filled. No separate email needed.",
    },
    {
      title: "Engagement Composite",
      desc: "Combines LMS logins, discussion posts, office hour visits, and submission timing into a single score per student.",
    },
  ],
};
    ...
    >
      ...
      <nav className="w-44 shrink-0 bg-navy-950 border-r border-white/[0.06] py-5 flex flex-col">
        ...
        {[
          ...
          { icon: Users, label: "My Advisors", on: false },
          { icon: Bell, label: "Alerts", on: false },
          { icon: BarChart2, label: "Analytics", on: false },
          { icon: FileText, label: "Report", on: false },
          { icon: Settings, label: "Settings", on: false },
        ].map((n) => (
          <button
            key={n.label}
            className={`relative flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium transition-colors
            ${
              n.on
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-5">
        ...
        <div className="flex flex-col gap-4">
          ...
          <CalloutCard
            calloutN={1}
            active={active === 1}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl bg-navy-900 border p-5 transition-all ${hi(1)}`}
          >
          ...
          <div className="rounded-xl bg-navy-900 border border-white/[0.06] p-4">
            <p className="text-[10px] text-slate-400 tracking-widest mb-3">
              GRADE TREND
            </p>
          ...
          <CalloutCard
            ...
            active={active === 3}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl bg-navy-900 border p-5 transition-all ${hi(3)}`}
          >
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <CalloutCard
            ...
            active={active === 2}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl bg-navy-900 border p-5 transition-all ${hi(2)}`}
          >
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] text-slate-400 tracking-widest">
                COURSE PERFORMANCE
            ...
            <div className="space-y-2">
              {COURSES.map((c) => (
                <div
                  key={c.code}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.04] transition-colors"
                >
                  <div
                    className={`w-2 h-2 rounded-full shrink-0 ${c.flag === "red" ? "bg-red-400" : "bg-teal-400"}`}
                  />
                  ...
                  <span
                    className={`text-xs font-bold shrink-0 w-9 text-right ${gradeColor(c.grade)}`}
                  >
                    {c.grade}%
                  </span>
                </div>
          ...
          <div className="rounded-xl bg-navy-900 border border-white/[0.06] p-5">
            ...
            <div className="space-y-2.5">
              {[
                {
                  sev: "red",
                  msg: "Missed 3 assignments in ECON 301",
                  time: "Today",
                },
                {
                  sev: "yellow",
                  msg: "Attendance below 75% in MATH 241",
                  time: "Yesterday",
                },
                {
                  sev: "yellow",
                  msg: "Grade dropped 8 pts in ECON 301",
                  time: "3 days ago",
                },
              ].map((a, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${a.sev === "red" ? "bg-red-400/15" : "bg-amber-400/15"}`}
                  >
                    <AlertTriangle
                      size={10}
                      className={
                        a.sev === "red" ? "text-red-400" : "text-amber-400"
                      }
                    />
                  </div>
          ...
          <CalloutCard
            ...
            active={active === 4}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl border p-5 transition-all ${active === 4 ? "bg-teal-500/5 border-teal-400 shadow-[0_0_0_2px_rgba(20,184,166,0.15)]" : "bg-navy-900 border-white/[0.06]"}`}
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-2.5">
              {[
                {
                  text: "Attend ECON 301 office hours this week",
                  tag: "Urgent",
                  tc: "red",
                },
                {
                  text: "Book MATH 241 tutoring center session",
                  tag: "This week",
                  tc: "amber",
                },
                {
                  text: "Meet with Dr. Lin — financial aid options",
                  tag: "Recommended",
                  tc: "teal",
                },
                {
                  text: "Submit 2 missing ECON 301 assignments",
                  tag: "Urgent",
                  tc: "red",
                },
              ].map((s, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <CheckCircle
                    size={13}
                    className="text-teal-500/30 shrink-0"
                  />
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div
                key={s.label}
                className="rounded-xl bg-navy-800/70 border border-white/[0.05] p-4"
              >
                ...
                <div
                  className={`flex items-center gap-0.5 mt-1.5 text-[10px] ${s.bad ? "text-red-400" : "text-teal-400"}`}
                >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl bg-navy-800/70 border border-white/[0.05] p-4">
              <p className="text-[9px] text-slate-400 tracking-widest mb-3">
            ...
            <CalloutCard
              ...
              active={active === 3}
              onToggle={(n) => set(active === n ? null : n)}
              className={`rounded-xl border p-4 transition-all ${hi(3)} bg-navy-800/70`}
            >
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="space-y-2">
                {[
                  {
                    sev: "red",
                    msg: "Alex Rivera — 3+ assignments missed",
                    time: "Today",
                  },
                  {
                    sev: "yellow",
                    msg: "Jordan Lee — attendance below threshold",
                    time: "Yesterday",
                  },
                  {
                    sev: "yellow",
                    msg: "Casey Nguyen — grade drop in 2+ courses",
                    time: "2 days ago",
                  },
                  {
                    sev: "blue",
                    msg: "Maya Patel — low discussion engagement",
                    time: "3 days ago",
                  },
                ].map((a, i) => (
          ...
          <CalloutCard
            ...
            active={active === 1}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl border p-4 transition-all bg-navy-800/70 ${active === 1 ? "border-teal-400 shadow-[0_0_0_2px_rgba(20,184,166,0.12)]" : "border-white/[0.05]"}`}
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <p className="text-[9px] text-slate-400 tracking-widest">
            ...
            {/* Column headers */}
            <div
              className="grid gap-2 px-2 py-1.5 text-[9px] text-slate-500 tracking-wider border-b border-white/[0.04] mb-1"
            ...
            {AT_RISK_STUDENTS.map((s, i) => (
              <div
                key={i}
                className="grid gap-2 items-center px-2 py-2 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer"
                style={{ gridTemplateColumns: "1fr 90px 110px 80px 24px" }}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <div
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${s.sev === "red" ? "bg-red-400" : "bg-amber-400"}`}
                  />
          ...
          <CalloutCard
            ...
            active={active === 4}
            onToggle={(n) => set(active === n ? null : n)}
            className={`rounded-xl border p-4 transition-all bg-navy-800/70 ${hi(4)}`}
          >
            <div className="flex items-center justify-between mb-3">
              <p className="text-[9px] text-slate-400 tracking-widest">
                INTERVENTION LOG
            ...
            <div className="space-y-3">
              {[
                {
                  who: "Alex Rivera",
                  what: "Email sent re: ECON 301 absences",
                  when: "Apr 24, 2:15 PM",
                  Icon: MessageSquare,
                },
                {
                  who: "Jordan Lee",
                  what: "Meeting scheduled — May 1 at 10:00 AM",
                  when: "Apr 23, 11:00 AM",
                  Icon: Calendar,
                },
                {
                  who: "Casey Nguyen",
                  what: "Referred to tutoring center for BIOL 202",
                  when: "Apr 22, 3:45 PM",
                  Icon: BookOpen,
                },
              ].map((l, i) => (
  ...
  return (
    ...
    >
      ...
      <nav className="w-44 shrink-0 bg-navy-950 border-r border-white/[0.06] py-5 flex flex-col">
        ...
        {[
          { icon: Home, label: "Overview", on: false },
          { icon: BookOpen, label: "My Courses", on: true },
          { icon: Bell, label: "Alerts", on: false },
          { icon: BarChart2, label: "Analytics", on: false },
          { icon: FileText, label: "Gradebook", on: false },
          { icon: Users, label: "Students", on: false },
          { icon: Settings, label: "Settings", on: false },
        ].map((n) => (
          <div className="relative flex flex-col" key={n.label}>
            <button
              key={n.label}
              className={`relative flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium transition-colors
            ${
            ...
            { n.label === "My Courses" && (
              <div className="mt-1 space-y-1">
                {/* {["ECON 301", "CS 201", "MATH 241", "ENGL 102"].map((c) => ( */}
                {[
                  { code: "ECON 301", flag: "red", on: true },
                  { code: "CS 201", flag: "teal", on: false },
                  { code: "MATH 241", flag: "amber", on: false },
                  { code: "ENGL 102", flag: "green", on: false },
                ].map((c) => (
                    <div key={c.code} className={`flex items-center gap-2.5 text-[10px] hover:text-slate-200 transition-colors cursor-pointer ${c.on ? "text-slate-300 bg-gray-900 bg-opacity-20" : "text-slate-600"} py-2 pl-4 w-full`}> 
                      <div
                        className={`w-2 h-2 rounded-full shrink-0 ${c.flag === "red" ? "bg-red-400" : c.flag === "amber" ? "bg-amber-400" : c.flag === "green" ? "bg-green-400" : "bg-teal-400"}`}
                      />
                      {c.code}
                    </div> 
                ))}
      ...
      <div className="flex flex-col gap-4 p-5 w-full">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] text-teal-400 tracking-widest">
                  ECON 301 · SECTION 02 · SPRING 2026
                </span>
              </div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Prof. Marcus Webb · 38 enrolled
              </p>
            </div>
            <div className="flex items-center gap-5">
              <div className="text-center">
                <p className="text-2xl font-bold text-amber-400">74%</p>
                <p className="text-[9px] text-slate-400 tracking-wider mt-0.5">
                  AVG ATTENDANCE
                </p>
              </div>
        ...
        <div
          className={`rounded-xl bg-navy-900 border p-5 transition-all ${active === 3 || active === 4 ? "border-teal-400 shadow-[0_0_0_2px_rgba(20,184,166,0.12)]" : "border-white/[0.06]"}`}
        >
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] text-slate-400 tracking-widest">
              STUDENT PERFORMANCE
          ...
          {/* Headers */}
          <div
            className="grid gap-3 px-2 py-1.5 text-[9px] text-slate-500 tracking-wider border-b border-white/[0.04] mb-1"
          ...
          {CLASS_STUDENTS.map((s, i) => (
            <div
              key={i}
              className="grid gap-3 items-center px-2 py-2.5 rounded-lg hover:bg-white/[0.03] transition-colors"
              style={{ gridTemplateColumns: "1fr 64px 80px 120px 96px" }}
            >
              <span className="text-xs text-slate-200">{s.name}</span>
              <span className={`text-xs font-bold ${gradeColor(s.grade)}`}>
                {s.grade}%
              </span>
              <span className={`text-xs ${attColor(s.att)}`}>{s.att}%</span>
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs font-medium shrink-0 ${engColor(s.engage)}`}
                >
                  {s.engage}
                </span>
                <div className="flex-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${s.engage}%`,
                      backgroundColor: engBg(s.engage),
                    }}
                  />
                </div>
              </div>
...
const ROLES: { id: Role; label: string; desc: string }[] = [
  {
    id: "student",
    label: "Student",
    desc: "Success score, course flags, action plan",
  },
  {
    id: "advisor",
    label: "Advisor",
    desc: "At-risk list, alert queue, intervention log",
  },
  {
    id: "professor",
    label: "Professor",
    desc: "Class trends, engagement scores, one-click flags",
  },
];
    ...
    <main className="min-h-screen bg-[#07091a] py-20 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <p className="text-teal-400 text-xs font-semibold uppercase tracking-[0.2em] mb-3">
            Interactive Demo
          </p>
          ...
          <p className="text-slate-400 text-sm max-w-xl mx-auto leading-relaxed">
            Switch between roles to explore each perspective. Click a numbered
            marker to see what each feature does.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-8">
          {ROLES.map((r) => (
            ...
            >
              <p
                className={`text-sm font-semibold ${role === r.id ? "text-teal-400" : "text-slate-300"}`}
              >
                {r.label}
              </p>
        ...
        <div>
          {role === "student" && (
            <StudentView active={active} set={setActive} />
          )}
          {role === "advisor" && (
            <AdvisorView active={active} set={setActive} />
          )}
